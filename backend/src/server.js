    ...
    const getSearchFilter = async (searchTerm) => {
      ...
      words.forEach(word => {
        ...
        const group = expanded.map(() =>
          `(LOWER(r.title) LIKE ? OR LOWER(r.description) LIKE ? OR LOWER(CAST(r.ingredients AS CHAR)) LIKE ? OR LOWER(r.category_country) LIKE ? OR LOWER(r.category_type) LIKE ?)`
        ).join(' OR ');
